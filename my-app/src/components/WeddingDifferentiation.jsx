...
const WeddingDifferentiation = () => {
  ...
  const items = [
    ...
    {
        title: "다시 열어볼 때마다\n전해지는 진심",
        description: <>축의금 봉투에 다 담지 못한 하객들의 소중한 진심을 한 권에 담아드려요 <span className="text-highlight">평생 꺼내 볼 수 있는 특별한 기록이에요</span></>,
        points: [
            { 
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        >
          ...
          <span className="text-3xl md:text-4xl font-mj2 font-black block">
            <span className={`diff-title-line ${line1Visible ? 'visible' : ''}`}><span style={{ fontFamily: "'Prata', serif", fontWeight: 'bold', letterSpacing: '0.1em' }}>VUE PHOTOBUTH</span>는</span>
            <br />
            <span className={`diff-title-line ${line2Visible ? 'visible' : ''}`}>퀄리티 높은 기술에 정성을 담아,</span>
            <br />
            <span className={`diff-title-line ${line3Visible ? 'visible' : ''}`}>가장 완벽한 서비스를 제공합니다</span>
          </span>
